...
class SpectrogramParser(AudioParser):
    ...
    def parse_audio(self, audio_path):
        ...
        print "Audio length/max/min:  ", len(y), max(y), min(y)
        fig = plt.figure()
        plt.plot(y)
        plt.title('Waveform')
        ...
        fig = plt.figure()
        librosa.display.specshow(librosa.amplitude_to_db(D,ref=np.max), y_axis='log', x_axis='time', sr=16000, hop_length=160)
        ...
        fig = plt.figure()
        librosa.display.specshow(librosa.amplitude_to_db(spect,ref=np.max), y_axis='log', x_axis='time', sr=16000, hop_length=160)
        plt.title("Spectrogram after Log")
        plt.colorbar(format='%+2.0f dB')
        plt.tight_layout()
        plt.savefig("logspectrogram.png", dpi=300)
        sys.exit()
